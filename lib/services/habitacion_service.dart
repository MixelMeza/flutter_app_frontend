...
class HabitacionService {
  ...
  static Future<Map<String, dynamic>> getLikesInfo(int habitacionId) async {
    final dynamic data = await ApiService.get('/api/habitaciones/$habitacionId/likes', token: ApiService.authToken);
  ...
  static Future<void> like(int habitacionId) async {
    ...
    final resp = await http.post(uri, headers: defaultJsonHeaders(ApiService.authToken));
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      throw ApiException('Like failed', statusCode: resp.statusCode, body: resp.body);
    }
  }
  ...
  static Future<void> unlike(int habitacionId) async {
    final uri = Uri.parse('$baseUrl/api/habitaciones/$habitacionId/like');
    final resp = await http.delete(uri, headers: defaultJsonHeaders(ApiService.authToken));
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      throw ApiException('Unlike failed', statusCode: resp.statusCode, body: resp.body);
    }
  }

  /// Record a view. If [sessionUuid] is provided it will be used for anonymous sessions.
  static Future<void> recordView(int habitacionId, {String? sessionUuid}) async {
    ...
    final resp = await http.post(uri, headers: defaultJsonHeaders(ApiService.authToken));
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      throw ApiException('Record view failed', statusCode: resp.statusCode, body: resp.body);
    }
  }

  /// Get recent views for the authenticated user
  static Future<List<dynamic>> recentForMe({int limit = 20}) async {
    final dynamic data = await ApiService.get('/api/habitaciones/recent?limit=$limit', token: ApiService.authToken);
    if (data is List) return data;
    return List<dynamic>.from(data);
  }

  /// Get recent views for a session (anonymous)
  static Future<List<dynamic>> recentForSession(String sessionUuid, {int limit = 20}) async {
    final dynamic data = await ApiService.get('/api/habitaciones/recent/session?sessionUuid=${Uri.encodeComponent(sessionUuid)}&limit=$limit');
    if (data is List) return data;
    return List<dynamic>.from(data);
  }
  ...
  static Future<List<Habitacion>> fetchDestacados({int limit = 20}) async {
    final dynamic data = await ApiService.get('/api/habitaciones?destacado=true&limit=$limit', token: ApiService.authToken);
    if (data is List) {
      return data.map((e) => Habitacion.fromJson(Map<String, dynamic>.from(e))).toList();
    }
    return [];
  }
}

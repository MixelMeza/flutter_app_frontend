...
class ViewMap extends StatefulWidget {
  ...
  const ViewMap({Key? key, required this.initialPosition, this.title, this.ubicacion}) : super(key: key);
        ...
        actions: [
          if (widget.ubicacion != null)
            IconButton(
              ...
              onPressed: () async {
                final result = await Navigator.of(context).push<bool?>(MaterialPageRoute(builder: (_) => _EditLocationPage(ubicacion: widget.ubicacion!, initialPosition: widget.initialPosition)));
...
class _EditLocationPage extends StatefulWidget {
  final Map<String, dynamic> ubicacion;
  final LatLng initialPosition;
  const _EditLocationPage({Key? key, required this.ubicacion, required this.initialPosition}) : super(key: key);
...
class _EditLocationPageState extends State<_EditLocationPage> {
  ...
  void initState() {
    ...
    _direccionController = TextEditingController(text: _pickString(widget.ubicacion, ['direccion', 'ubicacion', 'address']));
    _departamento = _pickString(widget.ubicacion, ['departamento', 'region', 'state']);
  ...
  Future<LatLng> _determineInitialMapPosition() async {
    try {
      ...
      final pos = await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.best, timeLimit: const Duration(seconds: 6));
  ...
  Future<void> _reverseGeocodeAndFill(LatLng latlng) async {
    ...
    try {
      ...
      for (final c in components) {
        ...
        if (types.contains('administrative_area_level_1')) region = c['long_name'];
        if (types.contains('administrative_area_level_2')) province = c['long_name'];
        if (types.contains('sublocality') || types.contains('locality') || types.contains('neighborhood')) district ??= c['long_name'];
      }
  ...
  int? _extractId(Map m) {
    if (m.containsKey('id')) return (m['id'] is int) ? m['id'] as int : int.tryParse(m['id'].toString());
    if (m.containsKey('uuid')) return int.tryParse(m['uuid'].toString());
    if (m.containsKey('ubicacionId')) return int.tryParse(m['ubicacionId'].toString());
    return null;
  }
  ...
  Future<void> _save() async {
    ...
    if (id == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('ID de ubicación no disponible')));
      return;
    }
    ...
    try {
      final resp = await http.put(uri, headers: defaultJsonHeaders(token), body: json.encode(payload)).timeout(const Duration(seconds: 12));
      if (resp.statusCode >= 200 && resp.statusCode < 300) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error actualizando: $body')));
      }
    } catch (e) {
      ...
      body: Column(children: [
        Expanded(
          child: Stack(children: [
            GoogleMap(
              initialCameraPosition: _initialCamera ?? CameraPosition(target: _marker, zoom: 16.0),
              markers: {
                // Old marker (original location) — shown for reference
                Marker(markerId: const MarkerId('old'), position: widget.initialPosition, infoWindow: const InfoWindow(title: 'Ubicación anterior')),
                // New/working marker — updated on camera idle
                Marker(markerId: const MarkerId('new'), position: _cameraTarget ?? _marker),
              },
              onMapCreated: (c) {
            ...
            IgnorePointer(
              ignoring: true,
              child: Center(child: Icon(Icons.place, size: 44, color: Theme.of(context).colorScheme.secondary.withOpacity(0.9))),
            ),
          ]),
        ),
        Padding(
          padding: const EdgeInsets.all(12.0),
            ...
            Row(children: [Expanded(child: Text('Departamento: ${_departamento ?? '-'}')),],),
            const SizedBox(height: 4),
            Row(children: [Expanded(child: Text('Provincia: ${_provincia ?? '-'}')),],),
            const SizedBox(height: 4),
            Row(children: [Expanded(child: Text('Distrito: ${_distrito ?? '-'}')),],),
            const SizedBox(height: 4),
            Row(children: [Expanded(child: Text('País: ${_pais ?? '-'}')),],),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: ElevatedButton.icon(onPressed: _loading ? null : _save, icon: _loading ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.save), label: Text(_loading ? 'Guardando...' : 'Guardar cambios'))),
            ],)
          ],),
        ),
      ],),

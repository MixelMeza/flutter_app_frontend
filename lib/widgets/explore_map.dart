...
class _ExploreMapState extends State<ExploreMap> {
  ...
  Map<String, Map<String, dynamic>> _markerCards =
      {}; // cache for /api/residencias/{id}/card
  ...
  void initState() {
    ...
    _reloadSub = ExploreMap._reloadController.stream.listen((_) {
      ...
      _loadRemoteMarkers().catchError(
        (e) => debugPrint('[ExploreMap] reload error: $e'),
      );
    });
    // Prepare icons ahead of loading markers to ensure consistency
    _ensureMarkerIcons().catchError(
      (e) => debugPrint('[ExploreMap] icon prepare error: $e'),
    );
  ...
  Future<void> _updateSelectedMarkerScreenPosition() async {
    ...
    try {
      ...
      final idx = _lastMarkerData!.indexWhere(
        (e) => ('m_${e['id']}') == _selectedMarkerId,
      );
      if (idx == -1) return;
      final found = _lastMarkerData![idx];
      if (!found.containsKey('lat') || !found.containsKey('lng')) return;
      final pos = LatLng(
        (found['lat'] as num).toDouble(),
        (found['lng'] as num).toDouble(),
      );
      if (!mounted) return;
      ScreenCoordinate sc;
      try {
        ...
        if (kDebugMode)
          debugPrint('[ExploreMap] getScreenCoordinate failed: $e');
        return;
      }
      ...
      if (kDebugMode)
        debugPrint(
          '[ExploreMap] selected marker screen coordinate: dx=$dx dy=$dy',
        );
    } catch (e) {
  ...
  Future<void> _goToWithYOffset(
    ...
    try {
      ...
      try {
        ...
        if (kDebugMode)
          debugPrint('[ExploreMap] _goToWithYOffset animateCamera failed: $e');
      }
    } catch (e) {
      ...
      try {
        ...
        if (kDebugMode)
          debugPrint(
            '[ExploreMap] _goToWithYOffset fallback animateCamera failed: $e2',
          );
      }
    }
  }
  ...
  Future<void> _openInExternalMaps(LatLng pos, String? label) async {
    try {
      ...
      final uri = Uri.parse(
        'https://www.google.com/maps/search/?api=1&query=${pos.latitude},${pos.longitude}${qLabel.isNotEmpty ? '&query=${Uri.encodeComponent(qLabel)}' : ''}',
      );
  ...
  Future<void> _openDirectionsTo(LatLng dest) async {
    try {
      final origin = (_devicePosition != null)
          ? '${_devicePosition!.latitude},${_devicePosition!.longitude}'
          : '';
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No se puede abrir la ruta')),
        );
    }
  }
  ...
  Widget _buildBottomCard(
    ...
    final disponibles =
        card['habitaciones_disponibles'] ?? card['habitacionesDisponibles'];
                              ...
                              Column(
                                ...
                                children: [
                                  if (disponibles != null && total != null) ...[
                                    Text(
                                      '${disponibles}/${total} habitaciones',
                                      style: const TextStyle(
                                        color: Colors.black54,
                                        fontSize: 13,
                                      ),
  ...
  Future<void> _loadRemoteMarkers() async {
    ...
    try {
      ...
      for (final item in normalized) {
        final id = item['id'];
        if (id != null)
          _preloadCard(id).catchError(
            (e) => debugPrint('[ExploreMap] preload card $id failed: $e'),
          );
      }
    } catch (e) {
  ...
  Future<void> _preloadCard(dynamic id) async {
    ...
    try {
      final data = await api_service.ApiService.get(
        '/api/residencias/${id.toString()}/card',
      );
      if (data is! Map<String, dynamic>)
        throw Exception('Invalid card response');
      ...
      try {
        ...
        rebuilt.add(
          Marker(
            ...
            icon: (_selectedMarkerId == markerId)
                ? (_iconSelected ?? BitmapDescriptor.defaultMarker)
  ...
  Future<void> _ensureMarkerIcons() async {
    ...
    Future<Uint8List> _createBytes(Color outer, Color inner) async {
      ...
      final shadowRect = Rect.fromCenter(
        center: Offset(center.dx, center.dy + headRadius * 0.85),
              ...
              GoogleMap(
                ...
                onMapCreated: (GoogleMapController controller) {
                  ...
                  if (_pendingCameraUpdates.isNotEmpty) {
                    final updates = List<CameraUpdate>.from(
                                ...
                                if (selPos != null)
                                  Material(
                                    ...
                                    child: IconButton(
                                      ...
                                      onPressed: () async {
                                        await _openInExternalMaps(
                                          selPos!,
                                          (_selectedCard != null
                                              ? (_selectedCard!['nombre']
                                                        ?.toString() ??
                                                    '')
                                              : ''),
                                        );

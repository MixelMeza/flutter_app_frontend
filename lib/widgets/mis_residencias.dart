...
class _ResilientNetworkImageState extends State<_ResilientNetworkImage> {
  ...
  Widget build(BuildContext context) {
    ...
    final int cacheW = (widget.height != null ? (widget.height! * dpr).round() : (400 * dpr).round());
    return Image.network(
      widget.url!,
      fit: widget.fit,
      height: widget.height,
      cacheWidth: cacheW,
      loadingBuilder: (context, child, loadingProgress) => loadingProgress == null ? child : Container(color: AppColors.midnightBlue.withAlpha((0.04 * 255).round()), child: const Center(child: CircularProgressIndicator(strokeWidth: 2))),
      errorBuilder: (context, err, st) => Container(color: AppColors.midnightBlue.withAlpha((0.06 * 255).round()), child: Center(child: Icon(Icons.apartment, size: 56, color: AppColors.tan))),
    );
  }
}
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: isDarkTile
                            ? [BoxShadow(color: Colors.black.withAlpha(28), blurRadius: 6, offset: const Offset(0,2))]
            ...
            Column(children: [
              ...
              _flatAction('Ver en mapa', trailing: Icons.map, onTap: () {
                final loc = r['ubicacion'];
                final coords = _extractLatLng(loc);
                if (coords != null) {
                  Navigator.of(context).push(MaterialPageRoute(builder: (_) => ViewMap(initialPosition: coords, title: r['nombre'] ?? r['title'])));
                } else {

...
class _ResilientNetworkImageState extends State<_ResilientNetworkImage> {
  ...
  Future<void> _checkConnectivity() async {
    try {
      ...
      if (_globalHasNetworkCheckedAt != null &&
          _globalHasNetwork != null &&
          now.difference(_globalHasNetworkCheckedAt!).inSeconds < 10) {
        setState(() {
...
class MisResidenciasState extends State<MisResidencias>
  ...
  LatLng? _extractLatLng(dynamic loc) {
    ...
    if (loc is Map) {
      final lat = _extractDoubleFromMap(loc, [
        'lat',
        'latitud',
        'latitude',
        'y',
      ]);
      final lng = _extractDoubleFromMap(loc, [
        'lng',
        'lon',
        'longitud',
        'longitude',
  ...
  Widget _buildStatsCards() {
    ...
    final totalRooms = mapped.fold<int>(
      0,
      (a, e) => a + _toInt(e['roomsTotal']),
    );
    ...
    final items = [
      {
        ...
        'label': 'Residencias',
        'iconColor': isDark ? AppColors.iconOnDark : AppColors.maroon,
      },
      {
        'icon': Icons.meeting_room,
        'value': '$availableCount',
        'label': 'Disponibles',
        'iconColor': isDark ? AppColors.iconOnDark : Colors.green,
      },
      {
        'icon': Icons.bed,
        'value': '$totalRooms',
        'label': 'Habitaciones',
        'iconColor': isDark ? AppColors.iconOnDark : AppColors.midnightBlue,
      },
      {
        'icon': Icons.attach_money,
        'value': '\$${(totalIncome / 1000).round()}k',
        'label': 'Ingresos',
        'iconColor': isDark ? AppColors.iconOnDark : AppColors.lightBlue,
      },
    ];
  ...
  Widget _flatAction(
    ...
    return Builder(
      builder: (ctx) {
        ...
        final bg = isDarkTile
            ? Theme.of(ctx).cardColor.withOpacity(0.96)
            : AppColors.alabaster;
        final textColor = isDarkTile
            ? AppColors.alabaster
            : AppColors.midnightBlue;
        final iconColor = isDarkTile
      ...
      onTap: () async {
        ...
        if (token.isEmpty) {
          ...
          messenger.showSnackBar(
            const SnackBar(
              content: Text('Debes iniciar sesión para agregar una residencia'),
            ),
          );
          return;
        }
        ...
        final result = await navigator.push(
          MaterialPageRoute(
            builder: (_) =>
                AddResidenciaForm(createUseCase: usecase, jwt: token),
          ),
        );
        if (result != null && result is Map<String, dynamic>) {
          ...
          if (mounted) {
            ...
            messenger.showSnackBar(
              SnackBar(
                content: Text('${result['nombre'] ?? 'Residencia'} añadida'),
              ),
            );
          }
        }
        ...
        child: Column(
          ...
          children: [
            Align(
              ...
              child: Icon(Icons.add, size: 28, color: iconColor),
            ),
  ...
  Widget build(BuildContext context) {
    ...
    final mapped = auth.myResidencias.map<Map<String, dynamic>>((m) {
      return {
        ...
        'roomsOccupied': _toInt(
          m['habitacionesOcupadas'] ?? m['roomsOccupied'],
        ),
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            child: _buildPropietarioEstadisticas(),
          ),
          const SizedBox(height: 16),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16),

...
class AddResidenciaForm extends StatefulWidget {
  ...
  final String? jwt;
  const AddResidenciaForm({super.key, required this.createUseCase, this.jwt});
...
class _AddResidenciaFormState extends State<AddResidenciaForm> {
    ...
    try {
      ...
      if (p != null) {
        ...
        if (phone != null && phone.toString().isNotEmpty)
          _contactoCtrl.text = phone.toString();
        if (email != null && email.toString().isNotEmpty)
          _emailCtrl.text = email.toString();
      }
    } catch (_) {}
  }
  ...
  Future<String?> _editHorarioRecepcion() async {
    ...
    final existing = _selectedServicios.firstWhere(
      (e) => e.startsWith('Horario de recepción'),
      orElse: () => '',
    );
          ...
          builder: (ctx2, setState2) {
            Future<void> pickClose() async {
              final t = await showTimePicker(
                context: ctx2,
                initialTime: localClose ?? const TimeOfDay(hour: 22, minute: 0),
              );
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  ListTile(
                    title: const Text('Libre (24/7)'),
                    leading: Radio<String>(
                      value: 'libre',
                      groupValue: localMode,
                      onChanged: (v) => setState2(() => localMode = v!),
                    ),
                  ),
                  ListTile(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        ElevatedButton(
                          onPressed: () {
                            if (localMode == 'libre') {
                              Navigator.of(ctx2).pop('Libre');
                            } else if (localMode == 'none')
                              Navigator.of(ctx2).pop('');
                            else if (localMode == 'close' &&
                                localClose != null) {
                              String fmt(TimeOfDay t) =>
                                  '${t.hour.toString().padLeft(2, '0')}:${t.minute.toString().padLeft(2, '0')}';
                              Navigator.of(
                                ctx2,
                              ).pop('Cierra ${fmt(localClose!)}');
                            } else {
                              ScaffoldMessenger.of(ctx2).showSnackBar(
                                const SnackBar(
                                  content: Text('Selecciona la hora de cierre'),
                                ),
                              );
                            }
                          },
  ...
  Future<void> _reverseGeocodeAndFill(LatLng latlng) async {
    ...
    debugPrint(
      '[reverseGeocode] called with lat=${latlng.latitude}, lng=${latlng.longitude}',
    );
    final messenger = ScaffoldMessenger.of(context);
    if (googleMapsApiKey.isEmpty) {
      messenger.showSnackBar(
        const SnackBar(
          content: Text('Google Maps API key not set - cannot reverse-geocode'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        debugPrint(
          '[reverseGeocode] response body length: ${resp.body.length}',
        );
        final results = (data['results'] as List?) ?? [];
        debugPrint(
          '[reverseGeocode] status=${data['status']} results=${results.length}',
        );
        if (data['status'] == 'OK' && results.isNotEmpty) {
          ...
          for (final r in results) {
            ...
            for (final comp in comps) {
              ...
              debugPrint(
                '[reverseGeocode] component: types=$types long_name=$longName',
              );
              if (longName == null) continue;
              if (route == null && types.contains('route')) route = longName;
              if (streetNumber == null && types.contains('street_number'))
          ...
          debugPrint(
            '[reverseGeocode] collected -> route=$route, streetNumber=$streetNumber, distrito=$distrito, provincia=$provincia, departamento=$departamento',
          );
          ...
          try {
            final nomUrl = Uri.parse(
              'https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${latlng.latitude}&lon=${latlng.longitude}&accept-language=es',
            );
            debugPrint(
              '[reverseGeocode] trying Nominatim fallback URL: $nomUrl',
            );
            // Nominatim requires a valid User-Agent identifying the application and contact info.
            final nomResp = await http
                .get(
                  nomUrl,
                  headers: {
                    'User-Agent':
                        'app_movil_flutter/1.0 (contact: dev@yourdomain.com)',
                    'Accept': 'application/json',
                  },
                )
                .timeout(const Duration(seconds: 8));
            debugPrint(
              '[reverseGeocode] nominatim status=${nomResp.statusCode} bodyLen=${nomResp.body.length}',
            );
            if (nomResp.statusCode == 200) {
              ...
              if (addr != null) {
                ...
                setState(() {
                  ...
                  _distritoLocked = (_distritoCtrl.text.trim().isNotEmpty);
                  _provinciaLocked = (_provinciaCtrl.text.trim().isNotEmpty);
                  _departamentoLocked = (_departamentoCtrl.text
                      .trim()
                      .isNotEmpty);
                });
                debugPrint(
                  '[reverseGeocode] nominatim collected -> direccion=${_direccionCtrl.text}, distrito=${_distritoCtrl.text}, provincia=${_provinciaCtrl.text}, departamento=${_departamentoCtrl.text}',
                );
              ...
              debugPrint(
                '[reverseGeocode] nominatim response body: ${nomResp.body}',
              );
            }
          } catch (e) {
          ...
          if (mounted)
            messenger.showSnackBar(
              SnackBar(
                content: Text(
                  gmsg == null
                      ? 'No se encontró dirección para la ubicación seleccionada'
                      : 'Error: $gmsg',
                ),
              ),
            );
        ...
        if (mounted)
          messenger.showSnackBar(
            SnackBar(content: Text('Error geocoding: ${resp.statusCode}')),
          );
      }
    } catch (e) {
      messenger.showSnackBar(
  ...
  Future<void> _submit() async {
    ...
    if (_pickedLatLng == null) {
      ...
      messenger.showSnackBar(
        const SnackBar(
          content: Text('Selecciona la ubicación en el mapa antes de crear'),
        ),
      );
      return;
    }
    ...
    final residencia = Residencia(
      ...
      tipo: _tipo,
      reglamentoUrl: _reglamentoCtrl.text.trim().isEmpty
          ? null
          : _reglamentoCtrl.text.trim(),
      servicios: _selectedServicios.isEmpty
          ? null
          : _selectedServicios.toList(),
      ubicacion: _pickedLatLng == null
          ? null
          : Ubicacion(
              ...
              distrito: _distritoCtrl.text.trim().isEmpty
                  ? null
    ...
    try {
      final result = await widget.createUseCase.call(
        residencia,
        jwt: widget.jwt ?? '',
      );
      if (mounted) {
        messenger.showSnackBar(
          const SnackBar(content: Text('Residencia creada')),
        );
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        spacing: 8,
                        runSpacing: 8,
                        children: _tipoIcons.keys.map((t) {
                          final sel = (_tipo == t);
                      ...
                      ElevatedButton.icon(
                        onPressed: () async {
                          ...
                          final latlng = await navigator.push<LatLng?>(
                            MaterialPageRoute(
                              builder: (_) =>
                                  _FullScreenMapPicker(initial: initial),
                            ),
                          );
                          if (latlng != null) {
                            ...
                            Expanded(
                              child: Text(
                                _pickedLatLng == null
                                    ...
                                    : 'Ubicación: ${_pickedLatLng!.latitude.toStringAsFixed(4)}, ${_pickedLatLng!.longitude.toStringAsFixed(4)}',
                              ),
                            ),
                      ...
                      TextFormField(
                        ...
                        decoration: const InputDecoration(
                          ...
                          labelText: 'Dirección (ej. Av. Universitaria 100)',
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.all(Radius.circular(12)),
                          ),
                        ),
                          ...
                          Expanded(
                            child: TextFormField(
                              ...
                              decoration: InputDecoration(
                                ...
                                suffixIcon: _departamentoLocked
                                    ? IconButton(
                                        icon: const Icon(Icons.edit, size: 18),
                                        onPressed: () => setState(
                                          () => _departamentoLocked = false,
                                        ),
                                      )
                                    : null,
                              ),
                          ...
                          Expanded(
                            child: TextFormField(
                              ...
                              decoration: InputDecoration(
                                ...
                                suffixIcon: _provinciaLocked
                                    ? IconButton(
                                        icon: const Icon(Icons.edit, size: 18),
                                        onPressed: () => setState(
                                          () => _provinciaLocked = false,
                                        ),
                                      )
                                    : null,
                              ),
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          suffixIcon: _distritoLocked
                              ? IconButton(
                                  icon: const Icon(Icons.edit, size: 18),
                                  onPressed: () =>
                                      setState(() => _distritoLocked = false),
                                )
                              : null,
                        ),
                      ),
                      ...
                      Wrap(
                        spacing: 10,
                        runSpacing: 10,
                        children: _availableServicios.map((s) {
                          final selected = _selectedServicios.any(
                            (e) => e == s || e.startsWith('$s:'),
                          );
                          final icon = _serviceIcons[s] ?? Icons.check_box;
                          // label may include horario detail
                          final match = _selectedServicios.firstWhere(
                            (e) => e == s || e.startsWith('$s:'),
                            orElse: () => '',
                          );
                          final label =
                              (match.isNotEmpty && match.startsWith('$s:'))
                              ? match
                              : s;
                          return GestureDetector(
                            onTap: () async {
                              if (s == 'Horario de recepción') {
                                ...
                                setState(() {
                                  _selectedServicios.removeWhere(
                                    (e) => e.startsWith('Horario de recepción'),
                                  );
                                  if (res != null && res.isNotEmpty)
                                    _selectedServicios.add(
                                      'Horario de recepción: $res',
                                    );
                                });
                                return;
                              }
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintText:
                              'Descripción breve: servicios, cercanía, extras',
                          filled: true,
                          border: const OutlineInputBorder(
                            borderRadius: BorderRadius.all(Radius.circular(12)),
                          ),
                      ...
                      TextFormField(
                        ...
                        decoration: const InputDecoration(
                          ...
                          labelText: 'Reglamento (URL o archivo)',
                          hintText: 'Por ahora pega la URL o deja vacío',
                        ),
                      ),
                      const SizedBox(height: 12),
                      ...
                      _loading
                          ...
                          : ElevatedButton.icon(
                              ...
                              label: const Text('Crear residencia'),
                            ),
                      const SizedBox(height: 8),
                    ],
                  ),
...
class _FullScreenMapPicker extends StatefulWidget {
  ...
  const _FullScreenMapPicker({required this.initial});
